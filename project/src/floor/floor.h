...
class Floor {
  ...
  public:
    void Init(GLuint tex_mirror = -1) {
        // compile the shaders
        program_id_ =
            icg_helper::LoadShaders("floor_vshader.glsl", "floor_fshader.glsl");
        if (!program_id_) {
            exit(EXIT_FAILURE);
        }
        ...
        // texture coordinates
        {
            ...
            // attribute
            GLuint vertex_texture_coord_id =
...

        {
            ...
            unsigned char *image =
                stbi_load(filename.c_str(), &width, &height, &nb_component, 0);
            ...
            GLuint tex_mirror_id =
                glGetUniformLocation(program_id_, "tex_mirror");
